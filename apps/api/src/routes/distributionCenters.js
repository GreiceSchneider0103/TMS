  ...
  app.patch('/distribution-centers/:id', requireAnyRole(['admin'], async ({ ctx, params, body }) => {
    const { rows } = await query(
      `update app.distribution_centers
       set name = coalesce($3, name), postal_code = coalesce($4, postal_code), city = coalesce($5, city), state = coalesce($6, state),
           address_line = coalesce($7, address_line), operating_hours = coalesce($8, operating_hours), is_active = coalesce($9, is_active), updated_at = now()
       where account_id = $1 and id = $2 and deleted_at is null returning *`,
      [ctx.accountId, params.id, body.name, body.postalCode, body.city, body.state, body.addressLine, body.operatingHours, body.isActive]
    );
    if (!rows[0]) throw new Error('Distribution center not found');
    return { ...rows[0], correlationId: ctx.correlationId };
  }));
